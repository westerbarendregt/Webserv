...
VirtualServer::t_routes::iterator	VirtualServer::getLocation(std::string const & path) {
	...
	while (it != this->m_configs.m_routes.end()) {
		...
		len = location_len < path.size() ? location_len : path.size();
		...
		found = it->first.find(path.c_str(), 0, len);
		if (found == 0) {
			if (it->first == "/") 
				return it; //fallback
			if (location_len > path.size() && it->first[path.size()] == '/')
				return it; // location: directory/ uri: directory
			if (path.size() == location_len)
				return it; //strictly equal
			if (location_len < path.size())
